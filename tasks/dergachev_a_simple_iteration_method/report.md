...
Метод простых итераций является классическим итерационным методом решения систем линейных алгебраических уравнений. В отличие от прямых методов, итерационные методы позволяют получить приближенное решение с заданной точностью и хорошо подходят для распараллеливания, особенно для больших разреженных матриц.
...
**Таблица 2: pipeline — Конвейерный режим выполнения**
...
*Примечание: Эффективность > 100% объясняется оптимизацией структуры данных в MPI версии*
...
1. Использование неблокирующих операций (`MPI_Isend`/`MPI_Irecv`) для перекрытия вычислений и коммуникаций
2. Увеличение размера задачи для лучшего соотношения вычислений к коммуникациям
...
3. Эффективность параллелизации составляет **85-92%** для базового режима 
...
1. Копченова Н.В., Марон И.А. *Вычислительная математика в примерах и задачах*. — М.: Лань, 2009.
2. MPI Forum. *MPI: A Message-Passing Interface Standard, Version 4.0*. — 2021. URL: https://www.mpi-forum.org/
3. Бахвалов Н.С., Жидков Н.П., Кобельков Г.М. *Численные методы*. — М.: Лаборатория знаний, 2015.
