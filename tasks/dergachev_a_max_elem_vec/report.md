...
|-------|-----------|------------|-----------|---------------|
| seq   | 1         | 0.2394     | 1.00      | N/A           |
| mpi   | 1         | 0.2293     | 1.04      | 104.4%        |
| mpi   | 2         | 0.1540     | 1.55      | 77.7%         |
| mpi   | 3         | 0.1449     | 1.65      | 55.1%         |
| mpi   | 4         | 0.1396     | 1.71      | 42.9%         |
...
1. **1 процесс MPI:** Достигнуто ускорение 1.04× по сравнению с seq версией. Результаты практически идентичны, что подтверждает корректность реализации без значительных накладных расходов MPI при одном процессе.

2. **2 процесса:** Достигнуто ускорение 1.55× при эффективности 77.7%

3. **3 процесса:** Ускорение 1.65× при эффективности 55.1%. Продолжение роста производительности, но эффективность снижается из-за увеличения коммуникационных затрат.

4. **4 процесса:** Ускорение 1.71× при эффективности 42.9%. Максимальное ускорение среди протестированных конфигураций. Снижение эффективности объясняется:
   - Увеличением коммуникационных затрат (MPI_Bcast и MPI_Allreduce)
   - Overhead на запуск и синхронизацию процессов
   - Простыми вычислениями на элемент
   - Дисбалансом нагрузки при распределении остатка

**Масштабируемость:**

Алгоритм отображает стабильный рост производительности при увеличении количества процессов, хотя эффективность снижается. При более сложных вычислениях на элемент ожидается лучшая масштабируемость на большем количестве процессов.
