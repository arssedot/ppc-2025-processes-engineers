...
1. **1 процесс MPI:** В обоих режимах время практически равно seq версии (ускорение ~1.0×), что подтверждает минимальные накладные расходы MPI при одном процессе.

2. **2 процесса:** 
   - task_pipeline: ускорение 1.40× при эффективности 70.4%
   - task_run: ускорение 1.62× при эффективности 81.1%
   - task_run показывает лучшие результаты благодаря более эффективной организации вычислений

3. **3 процесса:** 
   - task_pipeline: ускорение 1.83× при эффективности 61.2% - максимальное ускорение в этом режиме
   - task_run: ускорение 1.58× при эффективности 52.6%
   - task_pipeline показывает хороший результат на 3 процессах

4. **4 процесса:** 
   - task_pipeline: ускорение 1.33× при эффективности 33.2% - снижение производительности
   - task_run: ускорение 1.68× при эффективности 41.9% - максимальное ускорение в этом режиме
   - Снижение эффективности связано с ростом коммуникационных затрат

**Особенности:**

- task_run режим показывает более стабильные результаты и лучшую эффективность на 2 и 4 процессах
- task_pipeline режим демонстрирует пиковую производительность на 3 процессах, но нестабильную масштабируемость
- Эффективность падает с ростом числа процессов из-за увеличения коммуникационных затрат (MPI_Bcast, MPI_Allreduce) относительно простых вычислений

**Масштабируемость:**

Алгоритм показывает умеренную масштабируемость. Оптимальное количество процессов зависит от режима: 3 для task_pipeline и 4 для task_run. При более сложных вычислениях на элемент ожидается лучшая масштабируемость.
...
3. **Производительность:** На векторе из 100 млн элементов достигнуто максимальное ускорение:
   - Pipeline режим: 1.83× на 3 процессах (эффективность 61.2%)
   - Task_run режим: 1.68× на 4 процессах (эффективность 41.9%)
