...
inline double RastriginFunc(double x, double y) {
  constexpr double kA = 10.0;
  constexpr double kPi = 3.14159265358979323846;
  return kA * 2.0 + (x * x - kA * std::cos(2.0 * kPi * x)) + (y * y - kA * std::cos(2.0 * kPi * y));
}

inline double SphereFunc(double x, double y) {
  return x * x + y * y;
}

inline double RosenbrockFunc(double x, double y) {
  constexpr double kA = 1.0;
  constexpr double kB = 100.0;
  return (kA - x) * (kA - x) + kB * (y - x * x) * (y - x * x);
}

inline double BealeFunc(double x, double y) {
  double t1 = 1.5 - x + x * y;
...
inline double BoothFunc(double x, double y) {
  double t1 = x + 2.0 * y - 7.0;
  double t2 = 2.0 * x + y - 5.0;
  return t1 * t1 + t2 * t2;
}

inline double MatyasFunc(double x, double y) {
  return 0.26 * (x * x + y * y) - 0.48 * x * y;
}

inline double SimpleQuadratic(double x, double y) {
  return (x - 2.0) * (x - 2.0) + (y - 3.0) * (y - 3.0);
}

inline double MultiModalFunc(double x, double y) {
  return std::sin(x) * std::cos(y) + 0.1 * (x * x + y * y);
}
...
class DergachevAMultistep2dParallelFuncTests : public ppc::util::BaseRunFuncTests<InType, OutType, TestType> {
 ...
 protected:
  void SetUp() override {
    ...
    switch (test_id) {
      ...
      case 5:
        ...
        input_data_.max_iterations = 800;
        expected_x_ = 0.0;
        expected_y_ = 0.0;
        expected_f_ = 0.0;
        break;
  ...
  bool CheckTestOutputData(OutType &output_data) final {
    ...
    bool x_ok = std::abs(output_data.x_opt - expected_x_) < kTolerance;
    bool y_ok = std::abs(output_data.y_opt - expected_y_) < kTolerance;
    bool f_ok = std::abs(output_data.func_min - expected_f_) < kTolerance;
...
TEST_F(DergachevAMultistep2dValidationTests, FullPipelineSEQ) {
  if (!ppc::util::IsUnderMpirun()) {
    ...
    EXPECT_NEAR(result.x_opt, 0.0, 1.0);
    EXPECT_NEAR(result.y_opt, 0.0, 1.0);
...
TEST_F(DergachevAMultistep2dValidationTests, SmallSearchAreaSEQ) {
  if (!ppc::util::IsUnderMpirun()) {
    ...
    auto &result = task->GetOutput();
    EXPECT_NEAR(result.x_opt, 2.0, 0.3);
    EXPECT_NEAR(result.y_opt, 3.0, 0.3);
  }
}
...
TEST_F(DergachevAMultistep2dValidationTests, MultiModalFunctionSEQ) {
  if (!ppc::util::IsUnderMpirun()) {
    InType input;
    input.func = MultiModalFunc;
    input.x_min = -3.0;
    input.x_max = 3.0;
    input.y_min = -3.0;
    input.y_max = 3.0;
...
TEST_F(DergachevAMultistep2dValidationTests, BealeFunctionMPI) {
  ...
  auto &result = task->GetOutput();
  EXPECT_NEAR(result.x_opt, 3.0, 1.5);
